...
import { TrendingUp, TrendingDown, Clock, Lock, Target, ShieldAlert, BarChart3, Percent } from "lucide-react";
...
const ResultBadge = ({ status, pnl }: { status: string; pnl: number | null }) => {
  if (status === 'triggered') {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Active positions and exit history generated by the Kelly-sized signal engine"
      />
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border text-muted-foreground text-xs">
                      ...
                      <th className="text-right py-2 pl-4 font-medium">Days Active</th>
                    </tr>
                  </thead>
                  <tbody>
                    {visibleActive.map((s) => {
                    ...
                    {visibleExits.map((s) => {
                      ...
                      return (
                        <tr key={s.id} className={rowClass}>
                          ...
                          <td className="text-right py-2.5 px-4">
                            <ResultBadge status={s.status} pnl={s.pnl_pct} />
                          </td>
                          <td className="text-right py-2.5 pl-4 tabular-nums text-muted-foreground">
      ...
      {error && (
        <p className="text-sm text-destructive text-center">
          Failed to load trade signals: {(error as Error).message}
        </p>
      )}
      ...
      <PaywallModal
        ...
        feature="Full Trading Signals History"
        requiredPlan="Pro"
      />
    </div>
  );

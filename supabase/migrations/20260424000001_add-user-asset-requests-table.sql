-- User asset request feedback table.
-- Captures requests for assets users couldn't find in search so admins
-- can triage and prioritise catalogue additions.
--
-- Admin RLS uses public.has_role(auth.uid(), 'admin'), matching the
-- helper defined in 20251029043554_*.sql (user_roles table).
...
-- Authenticated users can create requests (RLS requires user_id = auth.uid())
CREATE POLICY "Authenticated users create requests"
  ON public.user_asset_requests FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

-- Admins can view every request
CREATE POLICY "Admins view all requests"
  ON public.user_asset_requests FOR SELECT
  USING (public.has_role(auth.uid(), 'admin'));

-- Admins can update triage fields (status, reviewed_at, reviewed_by, admin_notes)

...
BEGIN
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'hybrid_score', a.hybrid_score, 'computed_score', a.computed_score,
    ...
    'price_change_pct', (
      WITH last_two AS (SELECT close::numeric AS close, date FROM prices p WHERE p.ticker = a.ticker ORDER BY date DESC LIMIT 2),
      ordered AS (SELECT close, row_number() OVER (ORDER BY date DESC) AS rn FROM last_two)
      SELECT ROUND(((MAX(close) FILTER (WHERE rn = 1) - MAX(close) FILTER (WHERE rn = 2))
                    / NULLIF(MAX(close) FILTER (WHERE rn = 2), 0)) * 100, 2)
      FROM ordered HAVING COUNT(*) = 2
    )
  )), '[]'::jsonb) INTO v_demo_assets
  FROM assets a WHERE a.ticker = ANY(demo_tickers);

  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', a.id, 'ticker', a.ticker, 'name', a.name, 'exchange', a.exchange, 'asset_class', a.asset_class
  )), '[]'::jsonb) INTO v_blurred_assets
  FROM (
    SELECT id, ticker, name, exchange, asset_class FROM assets
    WHERE NOT (ticker = ANY(demo_tickers)) AND hybrid_score IS NOT NULL
    ORDER BY hybrid_score DESC NULLS LAST LIMIT 50
  ) a;

  SELECT COUNT(*) INTO v_total_assets FROM assets;
  SELECT COUNT(*) INTO v_scored_assets FROM assets WHERE hybrid_score IS NOT NULL;

  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', t.id, 'name', t.name, 'score', t.score, 'is_demo', t.is_demo,
    'ai_summary', t.ai_summary, 'tickers', t.tickers, 'keywords', t.keywords,
  ...
  FROM (
    ...
    ORDER BY COALESCE(score, 0) DESC NULLS LAST LIMIT 30
  ) t;

  SELECT COUNT(*) INTO v_total_themes FROM themes;

  SELECT to_jsonb(s) INTO v_demo_signal FROM (
    ...
    FROM trade_signals WHERE status = 'active' ORDER BY created_at DESC LIMIT 1
  ) s;
  ...
  result := jsonb_build_object(
    ...
    'scored_asset_count', v_scored_assets,

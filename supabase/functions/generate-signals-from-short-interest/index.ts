...
serve(async (req) => {
  ...
  try {
    ...
    for (let i = 0; i < signals.length; i += batchSize) {
      ...
      if (insertError) {
        console.log('[SIGNAL-GEN-SHORT] Batch error (continuing):', insertError.message);
      } else {
        insertedCount += data?.length || 0;
      }
    }

    console.log(`[SIGNAL-GEN-SHORT] ✅ Upserted ${insertedCount} short interest signals (${signals.length - insertedCount} duplicates skipped)`);

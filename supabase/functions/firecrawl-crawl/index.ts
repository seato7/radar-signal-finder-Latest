...
serve(async (req) => {
  ...
  const rl = await enforceRateLimit(auth.admin, auth.userId, 'firecrawl-crawl', 20, 3600);
  if (rl) return rl;
  ...
  try {
    ...
    try {
      response = await fetch('https://api.firecrawl.dev/v1/crawl', {
        ...
        body: JSON.stringify({
          url: normalizedUrl, limit,
          maxDepth: options?.maxDepth, includePaths: options?.includePaths, excludePaths: options?.excludePaths,
    ...
    return new Response(JSON.stringify({ success: true, ...data, duration_ms: duration }),

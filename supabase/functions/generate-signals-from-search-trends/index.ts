...
serve(async (req) => {
  ...
  try {
    ...
    const { data: trends, error: trendsError } = await supabaseClient.from('search_trends').select('*').gte('period_start', new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString()).order('period_start', { ascending: false });
    if (trendsError) throw trendsError;
    console.log(`[SIGNAL-GEN-TRENDS] Found ${trends?.length || 0} search trend records`);
    if (!trends || trends.length === 0) {
      const duration = Date.now() - startTime;
      await logHeartbeat(supabaseClient, { function_name: 'generate-signals-from-search-trends', status: 'success', rows_inserted: 0, duration_ms: duration, source_used: 'search_trends' });
      return new Response(JSON.stringify({ message: 'No search trends to process', signals_created: 0 }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    console.log(`[SIGNAL-GEN-TRENDS] ✅ Upserted ${insertedCount} search interest signals`);
    const duration = Date.now() - startTime;
    await logHeartbeat(supabaseClient, { function_name: 'generate-signals-from-search-trends', status: 'success', rows_inserted: insertedCount, rows_skipped: signals.length - insertedCount, duration_ms: duration, source_used: 'search_trends' });
    return new Response(JSON.stringify({ success: true, trends_processed: trends.length, signals_created: insertedCount, duplicates_skipped: signals.length - insertedCount }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error) {
    console.error('[SIGNAL-GEN-TRENDS] ❌ Error:', error);
    const duration = Date.now() - startTime;
    const supabaseClient = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '');
    await logHeartbeat(supabaseClient, { function_name: 'generate-signals-from-search-trends', status: 'failure', duration_ms: duration, error_message: error instanceof Error ? error.message : 'Unknown error' });

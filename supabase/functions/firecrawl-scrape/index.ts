...
serve(async (req) => {
  ...
  const rl = await enforceRateLimit(auth.admin, auth.userId, 'firecrawl-scrape', 20, 3600);
  if (rl) return rl;
  ...
  try {
    ...
    return new Response(JSON.stringify({ success: true, data: data.data || data, duration_ms: duration }),

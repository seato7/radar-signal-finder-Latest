...
serve(async (req) => {
  ...
  try {
    ...
    const { data: supplyChain, error: supplyError } = await supabaseClient
    ...
    if (!supplyChain || supplyChain.length === 0) {
      return new Response(JSON.stringify({ message: 'No supply chain data to process', signals_created: 0 }), {
    ...
    for (const supply of supplyChain) {
      ...
      const signalData = {
        ticker: supply.ticker,
        signal_type: 'supply_chain_indicator',
        report_date: supply.report_date,
      ...
      signals.push({
        ...
        direction,
        magnitude,
        observed_at: new Date(supply.report_date).toISOString(),
        value_text: `${supply.signal_type}: ${supply.metric_name} ${changePct > 0 ? '+' : ''}${changePct.toFixed(1)}%`,
        ...
        raw: {
          signal_type: supply.signal_type,
          metric_name: supply.metric_name,
          metric_value: supply.metric_value,
          change_percentage: changePct,
          indicator: supply.indicator
        }
      });
    }

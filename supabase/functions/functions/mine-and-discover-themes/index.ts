...
- No generic themes like "Technology" or "Growth Stocks"`;
    ...
    const fullPrompt = `You are an expert market analyst specializing in identifying emerging investment themes from multi-source signal data.\n\n${prompt}`;
    const aiSuggestions = await callGeminiPro(fullPrompt, 2000);
    if (!aiSuggestions) throw new Error('Gemini returned no content');

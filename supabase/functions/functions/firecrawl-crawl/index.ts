...
/**
 * Firecrawl Crawl - Recursively scrape all pages on a website
 */
...
serve(async (req) => {
  ...
  try {
    ...
    if (!apiKey) {
      console.error('[firecrawl-crawl] FIRECRAWL_API_KEY not configured');
      return new Response(
    ...
    console.log(`[firecrawl-crawl] Crawl initiated, status: ${data.status} (${duration}ms)`);
    ...
    console.error(`[firecrawl-crawl] Exception: ${errorMessage} (${duration}ms)`);
